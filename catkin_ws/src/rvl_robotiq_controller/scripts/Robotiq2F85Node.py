...
def start_robotiq_backend(device, refresh_rate = 200, delayed_start = 5):
    ...
    if ur_mode < 5:
        ...
        while ur_mode < 5 and not rospy.is_shutdown():
            rospy.sleep(1)
        logger.log_warn('UR Arm powered. Waiting for updates...')
    else:
        logger.log_warn('UR Arm already powered. Waiting for updates...')
    rospy.sleep(3)
    ...
    while not rospy.is_shutdown():
        if ur_mode < 5:
            logger.log_warn('UR Arm not powered. Sleeping...')
            while ur_mode < 5 and not rospy.is_shutdown():
                limiter.sleep()
            # wait a little bit before updating again
            logger.log_success('UR Arm repowered. Waiting for updates...')
            rospy.sleep(3)
        else:
